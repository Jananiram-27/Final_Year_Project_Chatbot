...
const generateChatResponse = async (req, res) => {
  try {
    const { message, userName, detected_tone: detectedToneInput } = req.body;
    const userId = req.user.id;
    const safeUserName = sanitizeUserName(userName);
    const detectedTone = normalizeDetectedTone(detectedToneInput);
    ...
    const normalizedMessage = normalizeForSentiment(message);
...
function normalizeForSentiment(message) {
...
function adjustSentimentScore(message, baseScore) {
  ...
  if (/(depress+|depresssi+on|depres+d|depresed|depresd|hopeless|lonely|alone|no\s*body\s*talks|nobody\s*talks|no\s*one\s*talks|not\s*feeling\s*good)/.test(lowered)) {
    return Math.min(baseScore, -0.45);
  }

  // Study/exam stress patterns that VADER often rates neutral.
  if (/\b(exam|test)\b/.test(lowered) && /\b(not\s*study|haven'?t\s*stud(y|ied)|unprepared|fail|panic|scared|anxious|stressed)\b/.test(lowered)) {
    return Math.min(baseScore, -0.4);
  }

  if (/\bi\s*(dont|don't)\s*know\s*how\s*to\s*study\b|\bi\s*have\s*not\s*stud(y|ied)\b/.test(lowered)) {
    return Math.min(baseScore, -0.38);
  }

  // If VADER lands near-neutral, boost using explicit emotion terms.
  if (baseScore > -0.15 && baseScore < 0.15) {
    if (/\b(sad|saad|unhappy|down|upset|heartbroken|crying|depressed|depresed|depresd|miserable|worthless|empty)\b/.test(lowered)) {
      return -0.42;
    }
    if (/\b(anxious|anxiety|stressed|overwhelmed|worried|nervous|confused)\b/.test(lowered)) {
      return -0.35;
    }
    if (/\b(happy|hapy|happi|joyful|excited|great|awesome|awsm|calm|relaxed|grateful|good|gud)\b/.test(lowered)) {
      return 0.42;
    }
  }
...
function normalizeDetectedTone(tone) {
  const raw = String(tone || '').trim().toLowerCase();
  if (!raw) return 'neutral';

  if (/(happy|joy|excited|positive|calm|relaxed)/.test(raw)) return 'happy';
  if (/(sad|low|depress|down)/.test(raw)) return 'sad';
  if (/(angry|mad|frustrat|irritat)/.test(raw)) return 'angry';
  if (/(fear|scared|anxious|anxiety|nervous|panic)/.test(raw)) return 'anxious';
  if (/(surprise|surprised)/.test(raw)) return 'surprised';
  return 'neutral';
}
...
async function generateGroqReply(message, history, meta = {}) {
  ...
  const detectedTone = normalizeDetectedTone(meta.detectedTone);
  ...
  const toneInstruction = `Detected voice tone is "${detectedTone}". Align wording and empathy with this tone while staying natural.`;

  const messages = [
    { role: 'system', content: `${BASE_COUNSELOR_PROMPT(userName)} ${behaviorInstruction} ${toneInstruction}` },
    ...(history || []),
