...
if GEMINI_API_KEY:
    ...
    gemini_model = None
    print("[WARN] Gemini API key missing. Chat will use fallback response.")
...
MODEL_CANDIDATES = [
    os.path.join(BASE_DIR, "model_file.h5"),
    os.path.join(BASE_DIR, "model_file_30epochs.h5"),
]
...
for candidate in MODEL_CANDIDATES:
    if os.path.exists(candidate):
        try:
            ...
            print(f"⚠️ Failed loading model {candidate}: {error}")

if model is not None:
    print(f"[OK] Loaded model from {loaded_model_path}")
else:
    print(f"[WARN] No compatible model file found. Tried: {MODEL_CANDIDATES}")

if os.path.exists(CASCADE_PATH):
    face_detect = cv2.CascadeClassifier(CASCADE_PATH)
    print("[OK] Loaded Haar Cascade Face Detector")
else:
    face_detect = None
    print("[WARN] Haar Cascade file not found.")

class_labels = {0: "Angry", 1: "Disgust", 2: "Fear", 3: "Happy", 4: "Neutral", 5: "Sad", 6: "Surprise"}


def detect_faces_robust(gray: np.ndarray):
    attempts = [
        (gray, 1.1, 4, (40, 40)),
        (gray, 1.08, 3, (30, 30)),
        (gray, 1.15, 5, (50, 50)),
