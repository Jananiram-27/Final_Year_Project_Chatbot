...
class MindSpaceChatbot {
    ...
    async processMessage(message) {
        const aiReply = await this.getBackendAiReply(message);
        if (aiReply) {
    ...
    async getBackendAiReply(message) {
        const token = localStorage.getItem('authToken');
        if (!token) return null;

        try {
            const response = await fetch(`${this.apiConfig.backendApiUrl}/api/ai/chat`, {
                method: 'POST',
                headers: {
            ...
            console.error('Homepage chatbot backend error:', error);
            return null;
        }
    }
