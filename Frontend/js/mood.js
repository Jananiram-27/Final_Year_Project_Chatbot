...
// Function to capture the mood image
    ...
    async function analyzeMoodImage(formData) {
        try {
            ...
            let userMessage = error.message || 'Mood detection failed';

            if (normalized.includes('no face')) {
                userMessage = 'No face detected. Please center your face in good lighting and try again.';
            } else if (normalized.includes('model not loaded')) {
                userMessage = 'Emotion model is not loaded on the server. Please restart the ML service.';
            } else if (normalized.includes('face detector not loaded')) {
                userMessage = 'Face detector is unavailable on the server. Please verify ML service setup.';
            } else if (normalized.includes('invalid image')) {
