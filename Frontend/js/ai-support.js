document.addEventListener('DOMContentLoaded', function() {
    ...
    const apiConfig = window.ENV_CONFIG || {
        backendApiUrl: 'https://soulspace-backend-46bh.onrender.com',
...
// (Replace with your actual route path)
    ...
    const detectedToneBar = document.getElementById('detected-tone-bar');
    const detectedToneValue = document.getElementById('detected-tone-value');
    ...
    let latestDetectedTone = 'Neutral';
    let toneAnalysisInFlight = false;
    ...
    async function initializePage() {
        ...
        setupSpeechFeatures();
        updateDetectedToneUI(latestDetectedTone, 'default');
    }
    ...
    function getCurrentUserName() {
        const explicitUserName = (localStorage.getItem('userName') || '').trim();
        if (explicitUserName) return explicitUserName;

        const fallbackUserData = JSON.parse(localStorage.getItem('userData') || '{}');
        const derivedName = (fallbackUserData.firstName || fallbackUserData.userName || '').trim();
    ...
    async function requestMicrophoneAndStartListening() {
        try {
            ...
            // Start tone analysis in background using a separate short stream.
            captureVoiceSample().catch((error) => {
                console.error('Voice tone capture failed:', error);
            });
    ...
    async function sendMessage() {
        ...
        const isVoiceMessage = lastInputMode === 'voice';
        ...
        try {
            ...
            const detectedTone = isVoiceMessage ? getDetectedTone() : null;
            const aiData = await generateAIResponse(message, detectedTone);
            hideTypingIndicator();
            
            // Score matrum text-a thani thaniya vaanguroom
            const responseText = aiData.text || aiData; 
            const vaderScore = aiData.score !== undefined ? aiData.score : null;
            const usedTone = isVoiceMessage ? (aiData.tone || getDetectedTone()) : null;

            // Show both sentiment and detected voice tone.
            addMessage(responseText, 'ai', vaderScore, usedTone);
            ...
            addMessage(`I could not reach the chatbot backend right now (${escapeHtml(errorMessage)}). Please re-login and ensure backend is running on port 5000.`, 'ai');
        } finally {
            lastInputMode = 'text';
        }
    }
 ...
 // Moondravathu parameter 'vaderScore' add pannirukoom
    function addMessage(text, sender, vaderScore = null, tone = null) {
        ...
        if (sender === 'ai') {
            // First: AI response-a formatting (bold, lists, etc.) panniduroom
            formattedText = formatAIResponse(text);
            
           
            const safeScore = Number(vaderScore);
            ...
            if (typeof tone === 'string' && tone.trim().length > 0) {
                formattedText += `<span style="display: block; color: #7a7a7a; font-size: 11px; font-style: italic;">Voice Tone: ${escapeHtml(tone)}</span>`;
            }
    ...
    // Function to prepare context for AI fallback
    ...
    function getDetectedTone() {
        if (typeof latestDetectedTone === 'string' && latestDetectedTone.trim().length > 0) {
            return latestDetectedTone.trim();
        }
        ...
        if (currentMood && typeof currentMood.label === 'string' && currentMood.label.trim().length > 0) {
            return currentMood.label.trim().toLowerCase();
        }

        return 'neutral';
    }

    function getVoiceToneEndpoint() {
        if (apiConfig && typeof apiConfig.mlServiceUrl === 'string' && apiConfig.mlServiceUrl.trim().length > 0) {
            return apiConfig.mlServiceUrl.replace(/\/predict_emotion\/?$/i, '/api/analyze_voice');
        }
        return 'http://localhost:5055/api/analyze_voice';
    }

    function updateDetectedToneUI(tone, source = 'default') {
        const safeTone = (tone || 'Neutral').toString().trim() || 'Neutral';
        latestDetectedTone = safeTone;
        window.detectedTone = safeTone;

        if (detectedToneValue) {
            detectedToneValue.textContent = safeTone;
        }
        if (detectedToneBar) {
            detectedToneBar.setAttribute('data-source', source);
        }
    }

    async function analyzeVoiceTone(audioBlob) {
        if (!audioBlob || toneAnalysisInFlight) return;
        toneAnalysisInFlight = true;

        try {
            const formData = new FormData();
            const file = new File([audioBlob], 'voice-tone.webm', {
                type: audioBlob.type || 'audio/webm',
                lastModified: Date.now()
            });
            formData.append('audio', file);

            const response = await fetch(getVoiceToneEndpoint(), {
                method: 'POST',
                body: formData
            });

            if (!response.ok) {
                throw new Error(`Tone API failed (${response.status})`);
            }

            const data = await response.json();
            const tone = (data && data.detected_tone) ? data.detected_tone : 'Neutral';
            updateDetectedToneUI(tone, data?.method || 'api');
        } catch (error) {
            console.error('Voice tone analysis error:', error);
            updateDetectedToneUI('Neutral', 'fallback');
        } finally {
            toneAnalysisInFlight = false;
        }
    }

    async function captureVoiceSample() {
        if (typeof MediaRecorder === 'undefined') return;
        const toneStream = await navigator.mediaDevices.getUserMedia({ audio: true });
        const chunks = [];

        await new Promise((resolve, reject) => {
            let recorder;
            try {
                recorder = new MediaRecorder(toneStream);
            } catch (error) {
                toneStream.getTracks().forEach(track => track.stop());
                reject(error);
                return;
            }
            ...
            recorder.onstop = async () => {
                ...
                try {
                    if (chunks.length > 0) {
                        ...
                        await analyzeVoiceTone(audioBlob);
                    }
                    resolve();
                } catch (error) {
                    reject(error);
    ...
    async function generateAIResponse(userMessage, detectedTone = 'neutral') {
        try {
            console.log('Sending request to AI service...');
            
            // Prepare context for AI
            const context = prepareContext(userMessage);
            const history = getRecentHistoryForApi();
            
            // Send to primary Node AI route (hardcoded context engine)
            ...
            if (!response.ok) {
                ...
                if (response.status === 401 || response.status === 403) {
                    throw new Error('Authentication failed (401/403). Please login again.');
                }
           ...
           const data = await response.json();
            ...
            if (data.reply || data.response || data.success) {
                return { 
                    text: data.response || data.reply, // Backend sends 'reply' or 'response'
                    score: data.sentiment_score !== undefined ? data.sentiment_score : data.sentimentScore,
                    tone: detectedTone ? (data.detected_tone || data.detectedTone || detectedTone) : null
                };
            } else {
